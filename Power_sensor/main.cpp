...
int main(int argc, char* argv[])
{
	...
	while(1) {
		...
		for (i = 0; i < numLITTLEcores; i++)
			output +=  getNode->GetCPUCurFreq(0+i) + '\t' + getNode->GetCPUTemp(0+i) + '\t';//there is no temp for little
		
		output += to_string(getNode->kfcuV) + '\t' + to_string(getNode->kfcuA) + '\t' + to_string(getNode->kfcuW) + '\t';

		for (i = 0; i < numbigcores; i++)
                        output +=  getNode->GetCPUCurFreq(4+i) + '\t' + getNode->GetCPUTemp(4+i) + '\t';//temp for GPU
		
		output += to_string(getNode->armuV) + '\t' + to_string(getNode->armuA) + '\t' + to_string(getNode->armuW) + '\t';
		
		output += getNode->GetGPUCurFreq() + '\t' + getNode->GetCPUTemp(numLITTLEcores+numbigcores)/*temp for big 02*/ + '\t' + to_string(getNode->g3duV) + '\t' + to_string(getNode->g3duA) + '\t' + to_string(getNode->g3duW) + '\t' + to_string(getNode->memuV) + '\t' + to_string(getNode->memuA) + '\t' + to_string(getNode->memuW);
		cmd += output;
