...
do
    case $opt in
		...
		echo "-q [FREQEUNCIES] -> Specify frequencies in Hz, separated by commas. Range is determined by core type. Second cluster (if selected, then must be the LITTLE cores)."
...
COREs=($(awk '{if($1 == "processor") print $3}' /proc/cpuinfo)) #Get a list of cores (0..7)
echo "BIG_CHOSEN $BIG_CHOSEN"
echo "LITTLE_CHOSEN $LITTLE_CHOSEN"
echo "CORE_CHOSEN  $CORE_CHOSEN"
echo "MAX_CORE $MAX_CORE"
echo "MIN_CORE $MIN_CORE"

for i in $(seq 0 $((${#COREs[@]} - 5))) #only the little cores
do
	if [[ $LITTLE_CHOSEN == 0 ]];then
		break;
	fi

	if [[ -n "$LITTLE_CHOSEN" ]];then

		if [[ -z "$CORE_RUN" ]]; then # -z variable empty
			#echo "$i CORE_RUN before $CORE_RUN"
			CORE_RUN="${COREs[$i]}"
			#echo "$i CORE_RUN after ${CORE_RUN[@]}"
		else
			...
			if [[ $(( $(echo "$CORE_RUN" | tr -cd ',' | wc -c) + 1 )) < $LITTLE_CHOSEN ]]; then
				CORE_RUN+=",${COREs[$i]}"
			#	echo "$i CORE_RUN (1) ${CORE_RUN[@]}"
			elif [[ -z "$CORE_HOTPLUG" ]]; then
	            CORE_HOTPLUG="${COREs[$i]}"
	         #   echo "$i CORE_HOTPLUG (2) ${CORE_HOTPLUG[@]}"
			else
				CORE_HOTPLUG+=",${COREs[$i]}"
			#	echo "$i CORE_HOTPLUG (3) ${CORE_HOTPLUG[@]}"
			fi
		fi	
...
for i in $(seq 4 $((${#COREs[@]} - 1))) #only the big cores
do
	if [[ $BIG_CHOSEN == 0 ]];then
		break;
	fi
	...
	if [[ -n "$BIG_CHOSEN" ]];then
		...
		if [[ -z "$CORE_RUN" ]]; then # -z variable empty
			...
			SUM=$BIG_CHOSEN
			[[ -n "$LITTLE_CHOSEN" ]] && SUM=$(echo "$BIG_CHOSEN+$LITTLE_CHOSEN" | bc)
			
			if [[ $(( $(echo "$CORE_RUN" | tr -cd ',' | wc -c) + 1 )) < $SUM  ]]; then
				CORE_RUN+=",${COREs[$i]}"
				#echo "$i CORE_RUN (1) ${CORE_RUN[@]}"
			elif [[ -z "$CORE_HOTPLUG" ]]; then
		        CORE_HOTPLUG="${COREs[$i]}"
...
echo "CORE_RUN (F) ${CORE_RUN[@]}"
echo "CORE_HOTPLUG (F) ${CORE_HOTPLUG[@]}"
...
cpufreq-set -d 200000 -u 200000 -c 4 -g performance
cpufreq-set -d 200000 -u 200000 -c 0 -g performance
...
IFS=' ' read -r -a CORE1_FREQ_ARRAY <<< "$CORE1_FREQ"
IFS=' ' read -r -a CORE2_FREQ_ARRAY <<< "$CORE2_FREQ"
#Run benchmarks for specified number of runs
for i in $(seq 1 "$NUM_RUNS");
do   
	echo "This is run $i out of $NUM_RUNS"      

	for FREQ_INDEX in $(seq 0 $((${#CORE1_FREQ_ARRAY[@]}-1)));
	do
		echo "FREQ_INDEX $FREQ_INDEX"

		FREQ_SELECT=${CORE1_FREQ_ARRAY[FREQ_INDEX]}
		FREQ2_SELECT=${CORE2_FREQ_ARRAY[FREQ_INDEX]}

		cpufreq-set -d "$FREQ_SELECT" -u "$FREQ_SELECT" -c 4 # the order of -b-L parameter matters!
